-- Core tables were never created on remote (migration was marked applied incorrectly).
-- The old handle_new_user() inserted into profiles(id, email) which no longer exists.
...
DROP POLICY IF EXISTS "own logs" ON public.glucose_logs;
CREATE POLICY "own logs" ON public.glucose_logs FOR ALL TO authenticated
  USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

CREATE INDEX IF NOT EXISTS idx_glucose_logs_user_time ON public.glucose_logs(user_id, logged_at DESC);
